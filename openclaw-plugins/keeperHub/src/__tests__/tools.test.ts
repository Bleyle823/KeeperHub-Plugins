...
function withFakeClient<T>(client: FakeClient, fn: () => Promise<T>): Promise<T> {
  ...
  const original = KeeperHubMcpClient.prototype.callTool;
  KeeperHubMcpClient.prototype.callTool = async function patched(
    this: KeeperHubMcpClient,
    name: string,
    args: Record<string, unknown> = {},
  ) {
...
describe('registerStatusTool', () => {
  it('reports NOT CONNECTED when no api key is configured', async () => {
    ...
    expect(result.content[0]!.text).toContain('plugins.entries.keeperHub.config.apiKey');
  });
