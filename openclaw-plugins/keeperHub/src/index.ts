  ...
  configSchema: {
    ...
    properties: {
      apiKey: {
        type: 'string',
        description:
          'KeeperHub organization API key (starts with kh_). Falls back to KH_API_KEY / KEEPERHUB_API_KEY env vars when omitted.',
      },
    },
  },

  register(api) {
    api.logger.info?.('[KeeperHub] Registering keeperHub plugin');

    if (!resolveApiKey(api)) {
      api.logger.warn?.(
        '[KeeperHub] No API key configured. Set plugins.entries.keeperHub.config.apiKey or KH_API_KEY env var. Tools will return a configuration error until a key is provided.',
      );
    }
