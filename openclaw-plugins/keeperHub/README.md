...
**Registry:** This package is published on **[ClawHub](https://docs.openclaw.ai/tools/clawhub)** as **`@keeperhub/openclaw-plugin`**. Installing by name pulls from ClawHub first (then npm fallback), which is usually all you need to add KeeperHub tools to OpenClaw.
...
```bash
openclaw plugins install clawhub:@keeperhub/openclaw-plugin
openclaw plugins install npm:@keeperhub/openclaw-plugin
```
...
If you maintain the plugin locally or forked [this repo](https://github.com/Bleyle823/Keepers-Eliza-Plugin), install from the directory path (**from your OpenClaw environment** — adapt the path to where `openclaw-plugins/keeperHub` lives on disk):

```bash
openclaw plugins install ./openclaw-plugins/keeperHub
openclaw gateway restart
```
...
openclaw config set plugins.entries.keeperHub.config.apiKey \
...
```bash
# From openclaw-plugins/keeperHub
bun install
bun test
...
- [KeeperHub Eliza plugin](../../packages/plugin-keeperHub) — ElizaOS port this plugin mirrors
