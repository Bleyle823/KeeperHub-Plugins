...
/** @deprecated Use {@link ApiKeyKind} `'webhook'` instead of `'workflow_builder'`. */
export type ApiKeyKindLegacy = ApiKeyKind | 'workflow_builder';
...
/** Classify key by known KeeperHub prefixes. */
...
/** True when the key is an organization key suitable for MCP (`kh_`). */
...
/**
 * Human-readable warning for status UIs, or `null` when the key looks correct for MCP.
 */
...
/**
 * Fail fast before opening an MCP session if the key cannot work with MCP.
 */
...
/** Mask a key for status UIs: first 4 + last 4 characters. */
